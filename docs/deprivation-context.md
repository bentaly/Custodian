...
_A plain-English summary for sharing. Built and working on staging._
...
show the **range** across that area — e.g. _"Decile 1–9, typically around 3"_ — because a
...
2. We work out _where that is_ on the map (using free address/map services).
...
- **It covers the whole UK.** England, Scotland, Wales and Northern Ireland each publish
  their _own_ deprivation index, measured differently. We use all four and always label
...
- **It measures where the project is _delivered_, not where the charity is based.** An
...
- **It's honest when it can't be sure.** "London" is too big and varied to reduce to one
  ...
  vague ("the North of England") or a typo'd location is shown plainly as _"couldn't
  determine"_ rather than inventing a misleading figure. Staff can correct the location by
  hand if needed.
...
- a small colour bar showing how the area's neighbourhoods are spread from most to least
  deprived,
- which area it matched (e.g. _"Leeds · local authority · 488 neighbourhoods"_),
...
- **Different nations, different scales.** Deciles are only strictly comparable _within_ a
