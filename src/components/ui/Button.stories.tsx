    ...
    <div className="flex flex-col gap-4">
      {(['md', 'sm', 'xs'] as const).map((size) => (
        <div key={size} className="flex items-center gap-3">
          <span className="w-8 font-display text-label text-grey-400">{size}</span>
  ...
  render: () => (
    <div className="flex flex-col items-start gap-3">
      <Button variant="text" onClick={() => {}}>
        Check the submission → <span className="text-grey-400">(opens a drawer — button)</span>
      </Button>
      ...
      <ExternalTextLink href="#">
        Award record → <span className="text-grey-400">(goes somewhere — anchor)</span>
      </ExternalTextLink>
    </div>
  ),
}
