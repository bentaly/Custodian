...
const FILES = {
  ...
  ewRegion:
    'LSOA_(2021)_to_Built_Up_Area_to_Local_Authority_District_to_Region_(December_2022)_Lookup_in_England_and_Wales_v2.csv',
}
...
function buildEngland(geo: Map<string, Geo>): Row[] {
  ...
  return loadCsv(FILES.england).map((r): Row => {
    ...
    return {
      ...
      rank:
        Number(r['Index of Multiple Deprivation (IMD) Rank (where 1 is most deprived)']) || null,
...
async function upsert(rows: Row[]) {
  const valid = rows.filter(
    (r) =>
      r.code &&
      nationFromGssCode(r.code) &&
      Number.isFinite(r.decile) &&
      r.decile >= 1 &&
      r.decile <= 10,
  )
  const CHUNK = 1000
...
async function main() {
  ...
  for (const [name, build] of builders) {
    try {
      ...
      if (dryRun) {
        const valid = rows.filter(
          (r) => Number.isFinite(r.decile) && r.decile >= 1 && r.decile <= 10,
        )
